...
-- this list may need updated dependencies regarding: https://setup.rbxcdn.com/<YOUR HASH>-rbxPkgManifest.txt
local globalZip = {
	...
	{"shaders"           	, [[shaders]]};
	{"Plugins"           	, [[Plugins]]};
	{"content-luapackages"	, [[content/LuaPackages]]};
...
local function downloadVersion(version,domain,dir)
	for i = 1,#globalZip do
		...
		if not string.find(zipData,"AccessDenied") then
			local f = assert(io.open('temp','wb'))
			f:write(zipData)
			f:flush()
			f:close()
			unzip('temp',path(dir or '.',globalZip[i][2]))
			print('Unpacked :' .. globalZip[i][1])
		else
			print('Unable to find :' .. globalZip[i][1])
		end
	end
	os.remove('temp')
